...
export default async function InteressadosPage({
  ...
  const linkPagina = (p: number) => `/backoffice/interessados?situacao=${filtro}&pagina=${p}`;

  return (
    <div>
      <header className="bg-white border-b border-slate-200 px-8 py-4">
        <h1 className="text-lg font-semibold text-slate-900">Interessados</h1>
      </header>

      <div className="p-8 space-y-6">
        <p className="text-sm text-slate-500">
          {/* O link para Leads / CRM saiu: aquela tela é do painel do cliente,
              e daqui o super admin não deve cair lá. O contraste continua
              valendo como explicação. */}
          Quem pediu contato pelos planos do site e ainda não assinou. É
          diferente de Leads / CRM, que é quem quer comprar um lote.
        </p>
      ...
      <div className="flex flex-wrap gap-2">
        {FILTROS.map((f) => (
          <Link
            key={f.chave}
            href={`/backoffice/interessados?situacao=${f.chave}`}
